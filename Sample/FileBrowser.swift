...
class FileBrowser: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if currentPage == ""{
            selectedpatharr.append(newPath)
        }else{
            selectedpatharr.append(currentPage.appending("/\(files[indexPath.row])"))
        }
        
        var isDirectory: ObjCBool = false
        fileManager.fileExists(atPath: newPath, isDirectory: &isDirectory)
        fileManager.componentsToDisplay(forPath: newPath)
        ...
        if isDirectory.boolValue {
            ...
            cell.imageView?.image = UIImage(named: "\(newPath)/images/1.png")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let url = URL(fileURLWithPath: selectedpatharr[indexPath.row])
        if url.pathExtension == "zip"{
            ...
            do {
                let a = try fileManager.contentsOfDirectory(atPath: selectedpatharr[indexPath.row])
                print(a)
                currentPage = selectedpatharr[indexPath.row]
                files = a
                selectedpatharr = []
                tableView.reloadData()
            } catch {
                
                let strs:NSString = selectedpatharr[indexPath.row] as NSString
                let urlsss = URL(fileURLWithPath: strs as String)
                webview.loadFileURL(urlsss, allowingReadAccessTo: urlsss)
    ...
    @IBAction func backBTN(_ sender: UIBarButtonItem) {
        ...
        let str:NSString = currentPage as NSString
        currentPage = str.deletingLastPathComponent
        do{
            
            
            let a = try fileManager.contentsOfDirectory(atPath: currentPage)
            print(currentPage)
